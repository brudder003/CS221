...
def mainGame(movementInfo):
    ...
    while True:
        ...
        playerYPos = playery + IMAGES['player'][1].get_width() / 2
        playerXPos = playerx + IMAGES['player'][0].get_width() / 2
        print '\n playerXPos: ', playerXPos, 'playerYPos: ', playerYPos
        pipeXPos = upperPipes[0]['x'] + IMAGES['pipe'][0].get_width() / 2
        pipeYPos = 360 - abs(upperPipes[0]['y'])# + IMAGES['pipe'][1].get_width()
        print '\n pipeXPos: ', pipeXPos, 'pipeYPos: ', pipeYPos
        if abs(playerXPos - pipeXPos) <= 100: 
            if playerYPos - pipeYPos > 30:
                if playery > -2 * IMAGES['player'][0].get_height():
                    playerVelY = playerFlapAcc
                    playerFlapped = True
                    SOUNDS['wing'].play()
...
def getRandomPipe():
    ...
    pipeX = SCREENWIDTH + pipeDistance
